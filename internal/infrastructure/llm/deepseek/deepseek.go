...
// Adapter implementa ports.LLM usando la API compatible con OpenAI de DeepSeek.
// Reutiliza el SDK de OpenAI apuntando al endpoint de DeepSeek,
// ya que DeepSeek expone una API 100% compatible con el formato OpenAI.
type Adapter struct {
...
const (
	// defaultBaseURL es el endpoint base de la API de DeepSeek.
	defaultBaseURL = "https://api.deepseek.com"
	// defaultModel es el modelo por defecto de DeepSeek.
	defaultModel = "deepseek-chat"
)

// NewAdapter crea un nuevo adaptador de DeepSeek con la API key y modelo especificados.
// Opcionalmente acepta un baseURL personalizado para instancias privadas.
func NewAdapter(apiKey, model, baseURL string) *Adapter {
	if model == "" {
		model = defaultModel
	}
	if baseURL == "" {
...
// Chat envía una solicitud conversacional a DeepSeek y retorna la respuesta
// mapeada a los modelos de dominio. Soporta Tool Calling compatible con OpenAI.
func (a *Adapter) Chat(ctx context.Context, req models.ChatRequest) (models.ChatResponse, error) {
	...
	if err != nil {
		return models.ChatResponse{}, fmt.Errorf("deepseek: error en chat completion: %w", err)
	}
